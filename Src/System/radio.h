...
typedef enum
{
	...
	CMD_APPTCB4 = 0x14,		// 从上位机发送到手柄的命令4, 校准手柄, 进入校准第一阶段, 进入校准第二阶段, 保存校准结果并退出校准
	CMD_APPTCB5 = 0x15,		// 从上位机发送到手柄的命令5, 透传功能, 上位机发送数据给接收机
	CMD_APPTCB6 = 0x16,		// 从上位机发送到接收机的命令6, 设置8路舵机PWM值和电机转速,电机转速弧度每秒,开启2路电源对外输出
	CMD_APPTCB7 = 0x17,		// 从上位机发送到接收机的命令7, 读取状态信息, 包括:8路舵机PWM值和电机转速(rad/s),
	...
	RES_APPTCB6 = 0x1E,	// 接收机对APPTCB6的响应, 返回设置结果
	RES_APPTCB7 = 0x1F,	// 接收机对APPTCB7的响应, 返回读取状态信息, 包括:8路舵机PWM值和电机转速(rad/s),
